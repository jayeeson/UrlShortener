...
export async function sqlQuery<T>(pool: mysql.Pool, query: string, args?: any[]): Promise<T[]> {
...
export async function sqlAlter<T>(pool: mysql.Pool, query: string, args?: any[]): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    pool.query(query, args, (err, row) => {
      if (err) {
        reject(err);
